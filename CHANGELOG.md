...
### بهبود
- **توقف تلاش مجدد برای خطاهای نهایی**: وقتی API اسپات پلیر خطای «واترمارک تکراری» برمی‌گرداند، سفارش به عنوان `_spot_fatal_error` علامت‌گذاری می‌شود. از این پس باکس ادمین دیگه spinner و دکمه تلاش مجدد نشون نمی‌ده و به جای آن پیام خطا + «نیاز به بررسی دستی» نمایش داده می‌شود. endpoint AJAX هم بلافاصله خطای `fatal_error` برمی‌گرداند.
